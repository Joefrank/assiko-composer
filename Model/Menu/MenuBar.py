...
class MenuBar(Control):
    def __init__(self, width:int, menu_height:int, menu_item_spacing:int, menu_bg_color:tuple):
    ...
    def resize(self, new_width_ratio, new_height_ratio):
        ...
        for i, child in enumerate(self.children):
            ...
            print(f"Resizing menu {self.next_x_offset}")
        return self.size, children_sizes

    def draw(self, surface):
        self.screen = surface
        pygame.draw.rect(surface, self.menu_bg_color, self.rect)
        any_open = any(m.open for m in self.children)
        for m in self.children:
            m.draw(surface, hover_open=any_open)
